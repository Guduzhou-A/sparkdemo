...
object SingRecommend {
  ...
  def main(args: Array[String]): Unit = {
    ...
    println("开始获取用户听歌数据.....")
    val fileRegex = "/part-*"
    val ratingsData = spark.read.textFile(hdfsUrl + hdfsSingDir + hdfsSingFileName + fileRegex).rdd
    val parts = ratingsData.map(line => parseSing(line))
    ...
    val userTable = TableName.valueOf(hbaseSingTableName)
    //初始化表单
    ...
    try {
      try {
        ...
        arrs.map(row => {
          ...
          putDataTable(userId, ratingWArr, table, "singCountInfo")
        })
